...
function svb_get_order_by_invoice_id($invoice_id) {
    ...
    global $wpdb;
    $table = $wpdb->prefix . 'svb_orders';
    $like = '%' . $wpdb->esc_like(sprintf('"invoice_id":"%s"', $invoice_id)) . '%';
    $row = $wpdb->get_row($wpdb->prepare("SELECT * FROM {$table} WHERE payment LIKE %s ORDER BY id DESC LIMIT 1", $like), ARRAY_A);
    if (!$row) {
        return null;
    }

    if (isset($row['payment'])) {
        $row['payment'] = svb_orders_decode_payment($row['payment']);
    }
...
function svb_update_user_payment_state($uid, array $updates) {
    ...
    if ($existing) {
        ...
        if (function_exists('svb_pay_log') && svb_pay_should_log()) {
            svb_pay_log('payment_state.update', [
                'order_id' => $existing['order_id'] ?? 0,
                'payment_status' => $new_payment['status'] ?? '',
                'email_hash_prefix' => substr($data['email_hash'] ?? '', 0, 8),
                'session_id_prefix' => substr($data['session_id'] ?? '', 0, 8),
                'fingerprint_at_invoice_prefix' => substr($new_payment['fingerprint_at_invoice'] ?? '', 0, 8),
                'paid_fingerprint_prefix' => substr($new_payment['paid_fingerprint'] ?? '', 0, 8),
            ]);
        }
    }
}
...
function svb_update_order_payment_by_id($order_id, array $updates, array $fingerprints = []) {
    if (!$order_id || !svb_orders_table_exists()) {
        return new WP_Error('svb_orders_table_missing', 'Orders table missing');
    }
    ...
    global $wpdb;
    $table = $wpdb->prefix . 'svb_orders';
    $row = $wpdb->get_row($wpdb->prepare("SELECT id,payment,fingerprint_current FROM {$table} WHERE order_id = %d LIMIT 1", $order_id), ARRAY_A);
    if (!$row) {
        return new WP_Error('svb_order_not_found', 'Order not found');
    }
    ...
    $fingerprint_current = $fingerprints['current'] ?? ($row['fingerprint_current'] ?? '');
    if (isset($updates['fingerprint_at_invoice']) && $updates['fingerprint_at_invoice']) {
        $fingerprint_current = $updates['fingerprint_at_invoice'];
    }

    if (isset($updates['status']) && $updates['status'] === 'success' && empty($new_payment['paid_fingerprint'])) {
        if (!empty($new_payment['fingerprint_at_invoice'])) {
            $new_payment['paid_fingerprint'] = $new_payment['fingerprint_at_invoice'];
        } elseif ($fingerprint_current) {
            $new_payment['paid_fingerprint'] = $fingerprint_current;
        }
    }

    $updated = $wpdb->update($table, ['payment' => wp_json_encode($new_payment)], ['id' => $row['id']], ['%s'], ['%d']);
    if ($updated === false) {
        return new WP_Error('svb_order_payment_update_failed', 'Failed to update payment', ['db_error' => $wpdb->last_error]);
    }
